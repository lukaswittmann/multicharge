   ...
   subroutine get_arguments(input, model_id, input_format, grad, charge, epsilon, path, &
      ...
      do while (iarg < narg)
         ...
         case ("-m", "-model", "--model")
            ...
            else if (arg == "eeqbceps2025" .or. arg == "eeqbceps" .or. arg == "eeqbc-eps") then
               model_id = mchargeModel%eeqbceps2025
            else
               call fatal_error(error, "Invalid model")
               exit
            end if
         case("-eps", "--eps", "--epsilon")
            iarg = iarg + 1
            call get_argument(iarg, arg)
            if (.not. allocated(arg)) then
               call fatal_error(error, "Missing argument for epsilon")
               exit
            end if
            ! if (model_id /= mchargeModel%eeqbceps2025) then
            !    call fatal_error(error, "Epsilon can only be set for the eeqbceps2025 model")
            !    exit
            ! end if
            allocate (epsilon)
            read (arg, *, iostat=iostat) epsilon
            if (iostat /= 0) then
               call fatal_error(error, "Invalid epsilon value")
               exit
            end if
